...
$(function() {
  ...
  $(document).on('click', '.delete-record', function() {
    ...
    Swal.fire({
      ...
      if (result.value) {
        ...
        $.ajax({
          type: 'DELETE',
          url: `${baseUrl}users/tarif-delete/${user_id}`,
  ...
  $(document).on('click', '.edit-record', function() {
    ...
    // Get data
    $.get(`${baseUrl}users/tarif/${user_id}/edit`, function(data) {
  ...
  const fv = FormValidation.formValidation(addNewUserForm1, {
    ...
    var url = isEditMode ? `${baseUrl}users/tarif-update/${userId}` : `${baseUrl}users/tarif-store`;
