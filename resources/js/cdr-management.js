...
$(function() {
  ...
  $(document).on('click', '.delete-record', function() {
    ...
    Swal.fire({
      ...
      if (result.value) {
        ...
        $.ajax({
          type: 'DELETE',
          url: `${baseUrl}users/cdr-delete/${user_id}`,
          success: function(response) {
            window.location.href = `${baseUrl}users/cdr-list`;
            dt_user.draw();
          },
  ...
  $(document).on('click', '.edit-record', function() {
    ...
    // Get data
    $.get(`${baseUrl}users/cdr/${user_id}/edit`, function(data) {
  ...
  const fv = FormValidation.formValidation(addNewUserForm1, {
    ...
    var url = isEditMode ? `${baseUrl}users/cdr-update/${userId}` : `${baseUrl}users/cdr-store`;
