...
$(function() {
  ...
  $(document).on('click', '.delete-record', function() {
    ...
    Swal.fire({
      ...
      if (result.value) {
        ...
        $.ajax({
          type: 'DELETE',
          url: `${baseUrl}users/client-delete/${user_id}`,
  ...
  $(document).on('click', '.edit-record', function() {
    ...
    // Get data
    $.get(`${baseUrl}users/client/${user_id}/edit`, function(data) {
  ...
  const fv = FormValidation.formValidation(addNewUserForm1, {
    ...
    var url = isEditMode ? `${baseUrl}users/client-update/${userId}` : `${baseUrl}users/client-store`;
