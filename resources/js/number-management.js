...
$(function() {
  ...
  $(document).on('click', '.delete-record', function() {
    ...
    Swal.fire({
      ...
      if (result.value) {
        ...
        $.ajax({
          type: 'DELETE',
          url: `${baseUrl}users/number-delete/${user_id}`,
  ...
  $(document).on('click', '.edit-record', function() {
    ...
    // Get data
    $.get(`${baseUrl}users/number/${user_id}/edit`, function(data) {
  ...
  const fv = FormValidation.formValidation(addNewUserForm1, {
    ...
    var url = isEditMode ? `${baseUrl}users/number-update/${userId}` : `${baseUrl}users/number-store`;
