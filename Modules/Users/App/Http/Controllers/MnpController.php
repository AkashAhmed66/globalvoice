...
class MnpController extends Controller
{
  ...
  public function index()
  {
    ...
    $tableHeaders = $this->getTableHeader('mnp-list');
    $userGroups = $this->userGroupRepository->all();

    return view('users::mnp.index', compact('title', 'tableHeaders', 'ajaxUrl', 'userGroups'));
  }
      ...
      if (!empty($filters['search_info'])) {
          ...
          $query->where(function ($q) use ($search) {
              $q->where('number', 'like', "%{$search}%")
                ->orWhere('recipientRC', 'like', "%{$search}%")
                ->orWhere('donorRC', 'like', "%{$search}%")
                ->orWhere('nrhRC', 'like', "%{$search}%")
